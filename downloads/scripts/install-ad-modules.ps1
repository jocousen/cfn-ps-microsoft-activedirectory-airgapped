...
Write-Output 'Installing # # # NuGet (Disabled for offline) (Disabled for offline) (Disabled for offline) Package Provider'
Try {
    $Null = Install-PackageProvider -Name '# # # NuGet (Disabled for offline) (Disabled for offline) (Disabled for offline)' -MinimumVersion '2.8.5' -Force -ErrorAction Stop
} Catch [System.Exception] {
    Write-Output "Failed to install # # # NuGet (Disabled for offline) (Disabled for offline) (Disabled for offline) Package Provider $_"
    Exit 1
}
...
Foreach ($Module in $Modules) {
    Try {
        # # # Install-Module (Disabled for offline) (Disabled for offline) (Disabled for offline) -Name $Module.Name -RequiredVersion $Module.Version -ErrorAction Stop
    } Catch [System.Exception] {
